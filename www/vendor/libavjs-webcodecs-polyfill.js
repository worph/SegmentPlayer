    ...
    function codecs(encoders) {
        return __awaiter$8(this, void 0, void 0, function* () {
            ...
            for (const [avname, codec] of [
                ...
                ["libvpx-vp9", "vp09"],
                ["libvpx", "vp8"],
                // SegmentPlayer patch: expose audio decoders our sp-audio
                // libav.js variant ships for polyfill-fallback decode when
                // native WebCodecs rejects a codec.
                ["ac3", "ac-3"],
                ["eac3", "ec-3"],
                ["dca", "dts"],
                ["truehd", "truehd"],
        ...
        if (typeof codec === "string") {
            ...
            switch (codec) {
                ...
                case "vorbis":
                    ...
                    outCodec = "libvorbis";
                    break;
                // SegmentPlayer patch: map WebCodecs codec strings to libav
                // avcodec names for the extra audio decoders compiled into
                // our sp-audio libav.js variant.
                case "ac-3":
                    outCodec = "ac3";
                    break;
                case "ec-3":
            ...
            for (const frame of outputs) {
                ...
                switch (frame.format) {
                    ...
                    case libav.AV_SAMPLE_FMT_FLT:
                        format = "f32";
                        break;
                    // SegmentPlayer patch: upstream labels planar libav formats
                    // with the interleaved name (e.g. "f32" for AV_SAMPLE_FMT_FLTP),
                    // but stores the data as concatenated planes. That makes
                    // consumers' copyTo({format:"f32-planar"}) read every-other
                    // sample from plane 0 (since WebCodecs treats source "f32"
                    // as interleaved). The fix is to label the format correctly.
